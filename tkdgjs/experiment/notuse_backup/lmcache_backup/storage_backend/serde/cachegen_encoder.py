...
def encode_function(
    ...
    """
    import os
    log_vram = os.environ.get("LMCACHE_VRAM_LOG", "0") == "1"
    
    def get_mem():
        return torch.cuda.memory_allocated() / (1024**3) if torch.cuda.is_available() else 0.0
    
    def log_step(name, mem_before):
        if log_vram:
            mem_after = get_mem()
            print(f"[LMCACHE_VRAM] {name}: +{mem_after - mem_before:.4f}GB (total: {mem_after:.4f}GB)")
            return mem_after
        return mem_before
    ...
    # Step 1: _split_kv
    mem = log_step("01_split_kv", mem)
    fp_k, fp_v = _split_kv(kv)
    nchannels = fp_k.shape[-1]
    nlayers = fp_k.shape[0] + fp_v.shape[0]
    
    # Step 2: torch_quant_vectorized for key
    mem = log_step("02_quant_key", mem)
    new_key, max_tensors_key = torch_quant_vectorized(key_bins, fp_k)
    
    # Step 3: torch_quant_vectorized for value
    mem = log_step("03_quant_value", mem)
    new_value, max_tensors_value = torch_quant_vectorized(value_bins, fp_v)
    
    # Step 4: torch.cat for encode_input
    mem = log_step("04_cat_encode_input", mem)
    encode_input = torch.cat((new_key, new_value), dim=0).reshape(
        nlayers, chunk_size, nchannels
    )
    
    # Step 5: calculate_cdf
    mem = log_step("05_calculate_cdf", mem)
    ...
    # Step 6: output_buffer allocation
    mem = log_step("06_output_buffer", mem)
    ...
    # Step 7: encode_ntokens loop
    mem = log_step("07_encode_ntokens_start", mem)
    data_chunks = []
    for i in range(0, chunk_size, CGBasics.CACHEGEN_GPU_MAX_TOKENS_PER_CHUNK):
        ...
        if log_vram and i == 0:
            mem = get_mem()
            print(f"[LMCACHE_VRAM] 08_encode_ntokens_chunk: total: {mem:.4f}GB")
    
    # Final result
    mem_after = get_mem()
    if log_vram:
        print(f"[LMCACHE_VRAM] encode_function: input={mem_input:.4f}GB, output={mem_after:.4f}GB, increase={mem_after - mem_input:.4f}GB")
