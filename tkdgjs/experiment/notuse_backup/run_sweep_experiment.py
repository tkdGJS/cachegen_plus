...
def run_single_experiment(serde_type: str, prefill_size: int, gpu_mem_util: float) -> dict:
    ...
    def measure_during_request():
        while not measurement_done:
            snap = monitor.monitor.measure(is_cachegen=(serde_type == "cachegen"))
            elapsed = time.time() - request_start_time
            with measurement_lock:
                measurement_results.append({
                    "elapsed_sec": elapsed,
                    "used_vram_gb": snap.used_vram_gb,
                    "kv_usage_perc": snap.vllm_kv_usage_perc,
                    "num_running": snap.vllm_num_running,
                    "num_waiting": snap.vllm_num_waiting,
                })
            time.sleep(0.01)  # 10ms interval
    
    measure_thread = threading.Thread(target=measure_during_request)
    measure_thread.start()
    
    success, latency_data = send_request_and_measure(prefill_size)
    
    time.sleep(0.5)  # Wait a bit more to capture completion state
    ...
    if measurement_results:
        max_vram_during = max(r["used_vram_gb"] for r in measurement_results)
        max_kv_usage = max(r["kv_usage_perc"] for r in measurement_results)
        max_running = max(r["num_running"] for r in measurement_results)
        print(f"[During request] VRAM range: {min(r['used_vram_gb'] for r in measurement_results):.2f} - {max_vram_during:.2f} GB")
        print(f"[During request] KV_usage: 0 - {max_kv_usage:.2f}%, running: 0 - {max_running}")
    else:
        max_vram_during = 0
        max_kv_usage = 0
        max_running = 0
