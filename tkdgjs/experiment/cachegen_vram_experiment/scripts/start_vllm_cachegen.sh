#!/bin/bash
#===============================================================================
# Start vLLM with CacheGen serialization - KV cache compression enabled
#
# Usage: ./start_vllm_cachegen.sh
#===============================================================================
...
export LMCACHE_CONFIG_FILE="$PROJECT_DIR/configs/lmcache_cachegen.yaml"
...
echo "========================================"
echo "Starting vLLM with CacheGen mode"
echo "========================================"
echo "Model: $MODEL"
echo "Config: $LMCACHE_CONFIG_FILE"
echo "Log: $LOG_DIR/vllm_cachegen.log"
echo ""
...
for i in {1..40}; do
    if curl -s http://localhost:$PORT/v1/models &> /dev/null; then
        ...
        if [ "$KV_BACKEND" = "lmcache" ]; then
            ...
            # Check if CacheGen is enabled
            echo ""
            echo "Verifying CacheGen compression..."
            grep "remote_serde.*cachegen" "$PROJECT_DIR/configs/lmcache_cachegen.yaml" && \
                echo "SUCCESS: CacheGen compression enabled!" || \
                echo "Note: Using default serialization"
        else
...
echo "Check log: $LOG_DIR/vllm_cachegen.log"
tail -30 "$LOG_DIR/vllm_cachegen.log"
exit 1
